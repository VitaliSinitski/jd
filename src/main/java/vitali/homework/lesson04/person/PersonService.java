...
public class PersonService {
    ...
    public static void peopleGenerator(List<Person> personList) {
        ...
        for (int i = 0; i < PersonService.PEOPLE_AMOUNT; i++) {
            personList.add(new Person(names[random.nextInt(names.length)], surnames[random.nextInt(surnames.length)], random.nextInt(PersonService.MIN_AGE_OF_GROUP, PersonService.MAX_AGE_OF_GROUP + 1)));
        }
    }

    public static void removeIfOlder(List<Person> personList) {
        personList.removeIf(person -> person.getAge() > PersonService.MAX_AGE_ALLOWED);
    }
